# 社交聊天H5应用 - 前端项目开发文档

> 本文档详细描述前端项目的模块设计、功能实现、技术方案等

**版本**: v1.1
**创建日期**: 2025-11-06
**更新日期**: 2025-11-06
**项目类型**: H5 Web Application
**技术栈**: React 18 + TypeScript + Vite + Next UI

---

## 📋 目录

1. [项目概述](#1-项目概述)
2. [技术架构](#2-技术架构)
3. [项目结构](#3-项目结构)
4. [核心模块设计](#4-核心模块设计)
   - 4.1 [首页模块](#41-首页模块)
   - 4.2 [社区模块](#42-社区模块)
   - 4.3 [发布模块](#43-发布模块)
   - 4.4 [消息模块](#44-消息模块)
   - 4.5 [我的模块](#45-我的模块)
5. [路由设计](#5-路由设计)
6. [状态管理](#6-状态管理)
7. [组件库设计](#7-组件库设计)
8. [工具类设计](#8-工具类设计)
9. [API接口设计](#9-api接口设计)
10. [开发规范](#10-开发规范)
11. [开发计划](#11-开发计划)

---

## 1. 项目概述

### 1.1 项目简介

社交聊天H5应用是一个集社交、聊天、内容分享于一体的移动端Web应用，为用户提供流畅的社交体验。

### 1.2 核心功能模块

应用采用底部Tab导航，包含5个核心模块：

| 模块 | 图标 | 功能描述 |
|------|------|---------|
| 首页 | 🏠 | 推荐内容流、热门动态、好友动态 |
| 社区 | 👥 | 话题广场、兴趣圈子、用户发现 |
| 发布 | ➕ | 发布动态、图文/视频内容创作 |
| 消息 | 💬 | 私聊、群聊、消息通知 |
| 我的 | 👤 | 个人中心、设置、个人主页 |

### 1.3 用户流程

```
注册/登录 → 完善资料 → 浏览首页 →
  ├─ 查看推荐内容
  ├─ 浏览社区话题
  ├─ 发布动态内容
  ├─ 聊天互动
  └─ 管理个人信息
```

---

## 2. 技术架构

### 2.1 技术栈

```json
{
  "核心框架": "React 18.2",
  "开发语言": "TypeScript 5.0",
  "构建工具": "Vite 5.0",
  "包管理器": "pnpm 8.0",
  "UI框架": "Next UI 2.0",
  "状态管理": "Redux Toolkit 2.0",
  "路由管理": "React Router 6.0",
  "HTTP请求": "Axios 1.6",
  "WebSocket": "Socket.io-client 4.0",
  "CSS框架": "TailwindCSS 3.0",
  "动画库": "Framer Motion 10.0",
  "图标库": "React Icons / Lucide React",
  "日期处理": "dayjs 1.11",
  "工具库": "lodash-es 4.17"
}
```

### 2.1.1 Next UI 特性

Next UI 是基于 Tailwind CSS 和 Framer Motion 构建的现代化 React UI 库：

**核心优势**：
- ✅ **现代化设计**：美观的默认主题，支持深色模式
- ✅ **TypeScript优先**：完整的类型定义
- ✅ **移动端优化**：专为移动端设计，支持触摸手势
- ✅ **高性能**：基于 Tailwind CSS，体积小，性能好
- ✅ **动画流畅**：集成 Framer Motion，动画丝滑
- ✅ **可定制性强**：主题系统完善，易于定制
- ✅ **无障碍性**：遵循 WAI-ARIA 标准

**核心组件**：
- Button、Input、Textarea、Select
- Modal、Dropdown、Popover、Tooltip
- Card、Avatar、Badge、Chip
- Table、Pagination、Skeleton
- Tabs、Accordion、Navigation
- 等50+组件

### 2.2 架构设计

```
┌─────────────────────────────────────────┐
│           UI Components Layer           │
│  (展示组件 - 负责UI渲染)                  │
├─────────────────────────────────────────┤
│         Business Logic Layer            │
│  (自定义Hooks - 负责业务逻辑)             │
├─────────────────────────────────────────┤
│           State Management              │
│  (Redux Toolkit - 全局状态管理)          │
├─────────────────────────────────────────┤
│            Service Layer                │
│  (API Services - 负责数据请求)           │
├─────────────────────────────────────────┤
│             Utils Layer                 │
│  (工具类 - SS前缀命名)                    │
└─────────────────────────────────────────┘
```

### 2.3 设计原则

- ✅ **三层分离**：UI层、逻辑层、请求层严格分离
- ✅ **组件化**：高内聚低耦合的组件设计
- ✅ **类型安全**：TypeScript严格模式
- ✅ **代码复用**：工具类封装，组件复用
- ✅ **性能优化**：懒加载、虚拟滚动、缓存策略

---

## 3. 项目结构

### 3.1 完整目录结构

```
social-chat-h5/
├── public/                          # 静态资源
│   ├── favicon.ico
│   ├── logo.png
│   └── index.html
│
├── src/
│   ├── assets/                      # 资源文件
│   │   ├── images/                  # 图片资源
│   │   │   ├── avatar/              # 默认头像
│   │   │   ├── empty/               # 空状态图片
│   │   │   └── common/              # 通用图片
│   │   ├── icons/                   # 图标资源
│   │   └── styles/                  # 全局样式
│   │       ├── variables.css        # CSS变量
│   │       ├── reset.css            # 样式重置
│   │       └── common.css           # 公共样式
│   │
│   ├── components/                  # 通用组件
│   │   ├── Layout/                  # 布局组件
│   │   │   ├── TabBar/              # 底部导航栏
│   │   │   ├── NavBar/              # 顶部导航栏
│   │   │   └── MainLayout/          # 主布局
│   │   ├── SSDialog/                # 统一弹窗组件
│   │   ├── Avatar/                  # 头像组件
│   │   ├── ImageUpload/             # 图片上传
│   │   ├── VideoUpload/             # 视频上传
│   │   ├── Loading/                 # 加载组件
│   │   ├── Empty/                   # 空状态
│   │   ├── PullRefresh/             # 下拉刷新
│   │   ├── InfiniteScroll/          # 无限滚动
│   │   └── index.ts                 # 统一导出
│   │
│   ├── views/                       # 页面模块（5个核心模块）
│   │   │
│   │   ├── Home/                    # 【首页模块】
│   │   │   ├── index.tsx            # 首页主入口
│   │   │   ├── Home.module.css
│   │   │   ├── FeedList/            # 内容流列表
│   │   │   │   ├── index.tsx
│   │   │   │   └── FeedItem/        # 内容流单项
│   │   │   │       └── index.tsx
│   │   │   ├── RecommendUsers/      # 推荐用户
│   │   │   │   └── index.tsx
│   │   │   ├── HotTopics/           # 热门话题
│   │   │   │   └── index.tsx
│   │   │   └── types.ts
│   │   │
│   │   ├── Community/               # 【社区模块】
│   │   │   ├── index.tsx            # 社区主入口
│   │   │   ├── Community.module.css
│   │   │   ├── TopicList/           # 话题列表
│   │   │   │   ├── index.tsx
│   │   │   │   └── TopicCard/       # 话题卡片
│   │   │   │       └── index.tsx
│   │   │   ├── TopicDetail/         # 话题详情
│   │   │   │   ├── index.tsx
│   │   │   │   └── PostList/        # 话题下的帖子
│   │   │   │       └── index.tsx
│   │   │   ├── UserDiscover/        # 用户发现
│   │   │   │   └── index.tsx
│   │   │   └── types.ts
│   │   │
│   │   ├── Publish/                 # 【发布模块】
│   │   │   ├── index.tsx            # 发布主入口
│   │   │   ├── Publish.module.css
│   │   │   ├── PublishType/         # 发布类型选择
│   │   │   │   └── index.tsx
│   │   │   ├── TextPost/            # 文字发布
│   │   │   │   └── index.tsx
│   │   │   ├── ImagePost/           # 图片发布
│   │   │   │   ├── index.tsx
│   │   │   │   └── ImagePreview/    # 图片预览
│   │   │   │       └── index.tsx
│   │   │   ├── VideoPost/           # 视频发布
│   │   │   │   └── index.tsx
│   │   │   ├── PublishSettings/     # 发布设置（权限、话题等）
│   │   │   │   └── index.tsx
│   │   │   └── types.ts
│   │   │
│   │   ├── Message/                 # 【消息模块】
│   │   │   ├── index.tsx            # 消息主入口
│   │   │   ├── Message.module.css
│   │   │   ├── ConversationList/    # 会话列表
│   │   │   │   ├── index.tsx
│   │   │   │   └── ConversationItem/  # 会话项
│   │   │   │       └── index.tsx
│   │   │   ├── ChatRoom/            # 聊天室
│   │   │   │   ├── index.tsx
│   │   │   │   ├── MessageList/     # 消息列表
│   │   │   │   │   ├── index.tsx
│   │   │   │   │   └── MessageBubble/  # 消息气泡
│   │   │   │   │       └── index.tsx
│   │   │   │   └── MessageInput/    # 消息输入框
│   │   │   │       ├── index.tsx
│   │   │   │       └── Toolbar/     # 工具栏
│   │   │   │           └── index.tsx
│   │   │   ├── GroupChat/           # 群聊
│   │   │   │   └── index.tsx
│   │   │   ├── Notification/        # 通知消息
│   │   │   │   └── index.tsx
│   │   │   └── types.ts
│   │   │
│   │   ├── Profile/                 # 【我的模块】
│   │   │   ├── index.tsx            # 个人中心主入口
│   │   │   ├── Profile.module.css
│   │   │   ├── UserInfo/            # 用户信息卡片
│   │   │   │   └── index.tsx
│   │   │   ├── MyPosts/             # 我的动态
│   │   │   │   └── index.tsx
│   │   │   ├── MyFriends/           # 我的好友
│   │   │   │   └── index.tsx
│   │   │   ├── Settings/            # 设置
│   │   │   │   ├── index.tsx
│   │   │   │   ├── AccountSettings/ # 账号设置
│   │   │   │   │   └── index.tsx
│   │   │   │   ├── PrivacySettings/ # 隐私设置
│   │   │   │   │   └── index.tsx
│   │   │   │   └── AboutSettings/   # 关于设置
│   │   │   │       └── index.tsx
│   │   │   ├── EditProfile/         # 编辑资料
│   │   │   │   └── index.tsx
│   │   │   └── types.ts
│   │   │
│   │   └── Auth/                    # 认证模块
│   │       ├── Login/               # 登录
│   │       ├── Register/            # 注册
│   │       └── ForgotPassword/      # 忘记密码
│   │
│   ├── store/                       # Redux状态管理
│   │   ├── slices/
│   │   │   ├── authSlice.ts         # 认证状态
│   │   │   ├── userSlice.ts         # 用户状态
│   │   │   ├── homeSlice.ts         # 首页状态
│   │   │   ├── communitySlice.ts    # 社区状态
│   │   │   ├── publishSlice.ts      # 发布状态
│   │   │   ├── messageSlice.ts      # 消息状态
│   │   │   └── profileSlice.ts      # 个人状态
│   │   ├── index.ts                 # Store配置
│   │   └── hooks.ts                 # Redux Hooks
│   │
│   ├── services/                    # 服务层
│   │   ├── api/                     # API接口
│   │   │   ├── auth.ts              # 认证接口
│   │   │   ├── user.ts              # 用户接口
│   │   │   ├── post.ts              # 动态接口
│   │   │   ├── topic.ts             # 话题接口
│   │   │   ├── message.ts           # 消息接口
│   │   │   ├── upload.ts            # 上传接口
│   │   │   └── index.ts
│   │   ├── socket/                  # WebSocket
│   │   │   ├── index.ts             # Socket连接
│   │   │   ├── events.ts            # 事件定义
│   │   │   └── handlers.ts          # 事件处理
│   │   └── request.ts               # HTTP请求封装
│   │
│   ├── hooks/                       # 自定义Hooks
│   │   ├── useAuth.ts               # 认证Hook
│   │   ├── useSocket.ts             # Socket Hook
│   │   ├── useFeed.ts               # 内容流Hook
│   │   ├── usePost.ts               # 动态Hook
│   │   ├── useChat.ts               # 聊天Hook
│   │   ├── useInfiniteScroll.ts     # 无限滚动Hook
│   │   ├── useDebounce.ts           # 防抖Hook
│   │   └── useThrottle.ts           # 节流Hook
│   │
│   ├── utils/                       # 工具类（SS前缀）
│   │   ├── SSTimeUtil.ts            # 时间工具
│   │   ├── SSStringUtil.ts          # 字符串工具
│   │   ├── SSValidateUtil.ts        # 验证工具
│   │   ├── SSStorageUtil.ts         # 存储工具
│   │   ├── SSDictUtil.ts            # 字典工具
│   │   ├── SSFileUtil.ts            # 文件工具
│   │   ├── SSImageUtil.ts           # 图片工具
│   │   └── index.ts
│   │
│   ├── types/                       # TypeScript类型
│   │   ├── models/                  # 数据模型
│   │   │   ├── user.ts
│   │   │   ├── post.ts
│   │   │   ├── topic.ts
│   │   │   ├── message.ts
│   │   │   └── index.ts
│   │   ├── api.ts                   # API类型
│   │   ├── common.ts                # 公共类型
│   │   └── index.ts
│   │
│   ├── constants/                   # 常量定义
│   │   ├── index.ts                 # 通用常量
│   │   ├── routes.ts                # 路由常量
│   │   └── enums.ts                 # 枚举定义
│   │
│   ├── router/                      # 路由配置
│   │   ├── index.tsx                # 路由入口
│   │   └── routes.tsx               # 路由定义
│   │
│   ├── config/                      # 配置文件
│   │   ├── index.ts                 # 配置入口
│   │   └── env.ts                   # 环境变量
│   │
│   ├── App.tsx                      # 应用根组件
│   ├── main.tsx                     # 应用入口
│   └── vite-env.d.ts
│
├── .env.development                 # 开发环境变量
├── .env.production                  # 生产环境变量
├── .gitignore
├── .eslintrc.cjs
├── .prettierrc
├── tsconfig.json
├── vite.config.ts
├── package.json
├── pnpm-lock.yaml
└── README.md
```

---

## 4. 核心模块设计

## 4.1 首页模块

### 4.1.1 模块概述

首页是用户进入应用后的第一个页面，展示个性化推荐内容、好友动态、热门话题等。

### 4.1.2 功能清单

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 推荐内容流 | P0 | 基于算法的个性化内容推荐 |
| 下拉刷新 | P0 | 刷新获取最新内容 |
| 上拉加载 | P0 | 无限滚动加载更多 |
| 内容互动 | P0 | 点赞、评论、分享 |
| 热门话题 | P1 | 展示热门话题标签 |
| 推荐用户 | P1 | 推荐可能感兴趣的用户 |
| 搜索入口 | P1 | 全局搜索功能入口 |
| 内容筛选 | P2 | 按类型筛选内容（图文/视频） |

### 4.1.3 页面结构

```
┌─────────────────────────────────┐
│  NavBar (顶部导航)               │
│  [Logo] [搜索] [通知]            │
├─────────────────────────────────┤
│                                 │
│  HotTopics (热门话题)            │
│  [#话题1] [#话题2] [#话题3]      │
│                                 │
├─────────────────────────────────┤
│                                 │
│  FeedList (内容流)               │
│  ┌─────────────────────────┐   │
│  │ FeedItem                │   │
│  │  [头像] [用户名] [时间]  │   │
│  │  [内容文字]              │   │
│  │  [图片/视频]             │   │
│  │  [❤️点赞] [💬评论] [🔗分享] │   │
│  └─────────────────────────┘   │
│  ┌─────────────────────────┐   │
│  │ FeedItem                │   │
│  └─────────────────────────┘   │
│  ...                            │
│                                 │
├─────────────────────────────────┤
│  TabBar (底部导航)               │
└─────────────────────────────────┘
```

### 4.1.4 组件设计

#### 主组件（views/Home/index.tsx）
```typescript
export const Home: React.FC = () => {
  const { feeds, loading, hasMore, loadMore, refresh } = useFeed();

  return (
    <div className={styles.home}>
      <NavBar />
      <PullRefresh onRefresh={refresh}>
        <HotTopics />
        <FeedList
          feeds={feeds}
          loading={loading}
          hasMore={hasMore}
          onLoadMore={loadMore}
        />
      </PullRefresh>
    </div>
  );
};
```

#### FeedItem组件（内容流单项）
```typescript
interface FeedItemProps {
  feed: Feed;
  onLike: (feedId: string) => void;
  onComment: (feedId: string) => void;
  onShare: (feedId: string) => void;
}

export const FeedItem: React.FC<FeedItemProps> = ({
  feed,
  onLike,
  onComment,
  onShare
}) => {
  return (
    <div className={styles.feedItem}>
      <FeedHeader user={feed.user} time={feed.createdAt} />
      <FeedContent content={feed.content} images={feed.images} />
      <FeedActions
        likeCount={feed.likeCount}
        commentCount={feed.commentCount}
        isLiked={feed.isLiked}
        onLike={() => onLike(feed.id)}
        onComment={() => onComment(feed.id)}
        onShare={() => onShare(feed.id)}
      />
    </div>
  );
};
```

### 4.1.5 状态管理（homeSlice.ts）

```typescript
interface HomeState {
  feeds: Feed[];
  page: number;
  pageSize: number;
  hasMore: boolean;
  loading: boolean;
  refreshing: boolean;
  hotTopics: Topic[];
  recommendUsers: User[];
}

export const homeSlice = createSlice({
  name: 'home',
  initialState,
  reducers: {
    setFeeds: (state, action) => {
      state.feeds = action.payload;
    },
    appendFeeds: (state, action) => {
      state.feeds.push(...action.payload);
    },
    updateFeed: (state, action) => {
      const index = state.feeds.findIndex(f => f.id === action.payload.id);
      if (index !== -1) {
        state.feeds[index] = action.payload;
      }
    },
    toggleLike: (state, action) => {
      const feed = state.feeds.find(f => f.id === action.payload);
      if (feed) {
        feed.isLiked = !feed.isLiked;
        feed.likeCount += feed.isLiked ? 1 : -1;
      }
    }
  }
});
```

### 4.1.6 API接口

```typescript
// services/api/post.ts
export const postApi = {
  // 获取推荐内容流
  getFeeds: (params: { page: number; pageSize: number }) =>
    axios.get<Feed[]>('/api/v1/posts/feeds', { params }),

  // 点赞动态
  likePost: (postId: string) =>
    axios.post(`/api/v1/posts/${postId}/like`),

  // 取消点赞
  unlikePost: (postId: string) =>
    axios.delete(`/api/v1/posts/${postId}/like`),

  // 获取动态详情
  getPostDetail: (postId: string) =>
    axios.get<Post>(`/api/v1/posts/${postId}`)
};
```

---

## 4.2 社区模块

### 4.2.1 模块概述

社区模块是用户发现兴趣话题、加入圈子、浏览话题内容的核心区域。

### 4.2.2 功能清单

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 话题广场 | P0 | 展示所有话题分类 |
| 话题列表 | P0 | 分类下的话题列表 |
| 话题详情 | P0 | 话题介绍、成员、内容 |
| 关注话题 | P0 | 关注/取消关注话题 |
| 话题内容流 | P0 | 话题下的帖子列表 |
| 话题搜索 | P1 | 搜索感兴趣的话题 |
| 推荐话题 | P1 | 个性化话题推荐 |
| 热门话题榜 | P1 | 热度排行榜 |
| 用户发现 | P2 | 发现相同兴趣用户 |

### 4.2.3 页面结构

```
┌─────────────────────────────────┐
│  NavBar                         │
│  [社区] [搜索]                   │
├─────────────────────────────────┤
│  TabBar (分类Tab)                │
│  [推荐] [关注] [热门]            │
├─────────────────────────────────┤
│                                 │
│  TopicList (话题列表)            │
│  ┌─────────────────────────┐   │
│  │ TopicCard               │   │
│  │  [话题封面]              │   │
│  │  [话题名称]              │   │
│  │  [话题描述]              │   │
│  │  [参与人数] [关注按钮]   │   │
│  └─────────────────────────┘   │
│  ┌─────────────────────────┐   │
│  │ TopicCard               │   │
│  └─────────────────────────┘   │
│  ...                            │
│                                 │
└─────────────────────────────────┘
```

### 4.2.4 组件设计

#### TopicCard组件（话题卡片）
```typescript
interface TopicCardProps {
  topic: Topic;
  onFollow: (topicId: string) => void;
  onClick: (topicId: string) => void;
}

export const TopicCard: React.FC<TopicCardProps> = ({
  topic,
  onFollow,
  onClick
}) => {
  return (
    <div className={styles.topicCard} onClick={() => onClick(topic.id)}>
      <div className={styles.cover}>
        <img src={topic.cover} alt={topic.name} />
      </div>
      <div className={styles.info}>
        <h3>{topic.name}</h3>
        <p>{topic.description}</p>
        <div className={styles.meta}>
          <span>{topic.memberCount} 参与</span>
          <Button
            size="small"
            type={topic.isFollowed ? 'default' : 'primary'}
            onClick={(e) => {
              e.stopPropagation();
              onFollow(topic.id);
            }}
          >
            {topic.isFollowed ? '已关注' : '关注'}
          </Button>
        </div>
      </div>
    </div>
  );
};
```

#### TopicDetail组件（话题详情）
```typescript
export const TopicDetail: React.FC = () => {
  const { id } = useParams();
  const { topic, posts, loading } = useTopicDetail(id);

  return (
    <div className={styles.topicDetail}>
      <NavBar back title={topic?.name} />

      <TopicHeader topic={topic} />

      <Tabs>
        <Tabs.Tab title="最新" key="latest">
          <PostList posts={posts} />
        </Tabs.Tab>
        <Tabs.Tab title="热门" key="hot">
          <PostList posts={posts} />
        </Tabs.Tab>
      </Tabs>
    </div>
  );
};
```

### 4.2.5 状态管理（communitySlice.ts）

```typescript
interface CommunityState {
  topics: Topic[];
  followedTopics: Topic[];
  hotTopics: Topic[];
  currentTopic: Topic | null;
  topicPosts: {
    [topicId: string]: Post[];
  };
  loading: boolean;
}
```

### 4.2.6 API接口

```typescript
// services/api/topic.ts
export const topicApi = {
  // 获取话题列表
  getTopics: (params: { category?: string; page: number }) =>
    axios.get<Topic[]>('/api/v1/topics', { params }),

  // 获取话题详情
  getTopicDetail: (topicId: string) =>
    axios.get<Topic>(`/api/v1/topics/${topicId}`),

  // 关注话题
  followTopic: (topicId: string) =>
    axios.post(`/api/v1/topics/${topicId}/follow`),

  // 取消关注话题
  unfollowTopic: (topicId: string) =>
    axios.delete(`/api/v1/topics/${topicId}/follow`),

  // 获取话题下的帖子
  getTopicPosts: (topicId: string, params: { page: number }) =>
    axios.get<Post[]>(`/api/v1/topics/${topicId}/posts`, { params })
};
```

---

## 4.3 发布模块

### 4.3.1 模块概述

发布模块支持用户创作和发布多种类型的内容，包括文字、图片、视频等。

### 4.3.2 功能清单

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 文字发布 | P0 | 纯文字内容发布 |
| 图片发布 | P0 | 图文内容发布（最多9张） |
| 视频发布 | P0 | 视频内容发布 |
| 图片编辑 | P1 | 裁剪、滤镜、贴纸 |
| 添加话题 | P1 | 关联话题标签 |
| @好友 | P1 | 提及好友 |
| 位置标记 | P1 | 添加地理位置 |
| 权限设置 | P0 | 公开/好友可见/私密 |
| 草稿保存 | P2 | 保存未发布内容 |

### 4.3.3 页面结构

```
┌─────────────────────────────────┐
│  NavBar                         │
│  [取消] [发布]   [发送按钮]      │
├─────────────────────────────────┤
│                                 │
│  TextInput (文字输入)            │
│  ┌─────────────────────────┐   │
│  │ 分享新鲜事...            │   │
│  │                         │   │
│  │                         │   │
│  └─────────────────────────┘   │
│                                 │
│  MediaPreview (媒体预览)         │
│  ┌─────┐ ┌─────┐ ┌─────┐       │
│  │图片1 │ │图片2 │ │ + │        │
│  └─────┘ └─────┘ └─────┘       │
│                                 │
│  Toolbar (工具栏)                │
│  [📷图片] [🎬视频] [#话题] [@好友] [📍位置] │
│                                 │
│  Settings (发布设置)             │
│  谁可以看: [公开 ▼]              │
│  位置: [添加位置]                │
│  话题: [#选择话题]               │
│                                 │
└─────────────────────────────────┘
```

### 4.3.4 组件设计

#### PublishType组件（发布类型选择）
```typescript
export const PublishType: React.FC = () => {
  const navigate = useNavigate();

  const types = [
    { icon: '📝', label: '文字', path: '/publish/text' },
    { icon: '📷', label: '图片', path: '/publish/image' },
    { icon: '🎬', label: '视频', path: '/publish/video' }
  ];

  return (
    <div className={styles.publishType}>
      <div className={styles.typeGrid}>
        {types.map(type => (
          <div
            key={type.path}
            className={styles.typeItem}
            onClick={() => navigate(type.path)}
          >
            <div className={styles.icon}>{type.icon}</div>
            <span>{type.label}</span>
          </div>
        ))}
      </div>
    </div>
  );
};
```

#### ImagePost组件（图片发布）
```typescript
export const ImagePost: React.FC = () => {
  const [content, setContent] = useState('');
  const [images, setImages] = useState<File[]>([]);
  const [topic, setTopic] = useState<string>('');
  const [visibility, setVisibility] = useState<'public' | 'friends' | 'private'>('public');

  const handleUploadImage = (files: File[]) => {
    if (images.length + files.length > 9) {
      SSDialog.toast('最多只能上传9张图片', 'warning');
      return;
    }
    setImages([...images, ...files]);
  };

  const handlePublish = async () => {
    if (!content && images.length === 0) {
      SSDialog.toast('请输入内容或上传图片', 'warning');
      return;
    }

    try {
      // 上传图片
      const imageUrls = await uploadImages(images);

      // 发布动态
      await postApi.createPost({
        content,
        images: imageUrls,
        topic,
        visibility
      });

      SSDialog.toast('发布成功', 'success');
      navigate('/home');
    } catch (error) {
      SSDialog.toast('发布失败', 'error');
    }
  };

  return (
    <div className={styles.imagePost}>
      <NavBar
        back
        title="发布图片"
        right={
          <Button onClick={handlePublish}>发布</Button>
        }
      />

      <TextArea
        placeholder="分享新鲜事..."
        value={content}
        onChange={setContent}
        maxLength={500}
      />

      <ImageUpload
        images={images}
        onChange={setImages}
        maxCount={9}
      />

      <PublishSettings
        topic={topic}
        visibility={visibility}
        onTopicChange={setTopic}
        onVisibilityChange={setVisibility}
      />
    </div>
  );
};
```

### 4.3.5 状态管理（publishSlice.ts）

```typescript
interface PublishState {
  draft: {
    content: string;
    images: string[];
    video?: string;
    topic?: string;
    visibility: 'public' | 'friends' | 'private';
  } | null;
  uploading: boolean;
  uploadProgress: number;
}
```

### 4.3.6 API接口

```typescript
// services/api/post.ts
export const postApi = {
  // 创建动态
  createPost: (data: CreatePostDto) =>
    axios.post<Post>('/api/v1/posts', data),

  // 上传图片
  uploadImages: (files: File[]) =>
    axios.post<string[]>('/api/v1/upload/images', files),

  // 上传视频
  uploadVideo: (file: File) =>
    axios.post<string>('/api/v1/upload/video', file)
};
```

---

## 4.4 消息模块

### 4.4.1 模块概述

消息模块提供私聊、群聊、系统通知等即时通讯功能。

### 4.4.2 功能清单

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 会话列表 | P0 | 显示所有聊天会话 |
| 私聊 | P0 | 一对一聊天 |
| 群聊 | P0 | 群组聊天 |
| 文字消息 | P0 | 发送接收文字 |
| 图片消息 | P0 | 发送接收图片 |
| 语音消息 | P1 | 发送接收语音 |
| 视频消息 | P1 | 发送接收视频 |
| 消息撤回 | P1 | 2分钟内撤回 |
| 消息已读 | P0 | 已读未读状态 |
| 消息搜索 | P2 | 搜索聊天记录 |
| 系统通知 | P0 | 系统消息推送 |

### 4.4.3 页面结构

**会话列表页**
```
┌─────────────────────────────────┐
│  NavBar                         │
│  [消息]              [+新建]     │
├─────────────────────────────────┤
│  SearchBar (搜索)                │
├─────────────────────────────────┤
│                                 │
│  ConversationList               │
│  ┌─────────────────────────┐   │
│  │ [头像] 张三              │   │
│  │        你好               │   │
│  │        10:30  [2]        │   │
│  └─────────────────────────┘   │
│  ┌─────────────────────────┐   │
│  │ [群头像] 工作群          │   │
│  │          李四: 收到      │   │
│  │          昨天             │   │
│  └─────────────────────────┘   │
│  ...                            │
│                                 │
└─────────────────────────────────┘
```

**聊天室页**
```
┌─────────────────────────────────┐
│  NavBar                         │
│  [<返回] 张三         [•••]     │
├─────────────────────────────────┤
│                                 │
│  MessageList (消息列表)          │
│                                 │
│  ┌─────────────────┐            │
│  │ 对方消息气泡     │            │
│  └─────────────────┘            │
│            ┌─────────────────┐  │
│            │ 我的消息气泡     │  │
│            └─────────────────┘  │
│  ┌─────────────────┐            │
│  │ [图片消息]       │            │
│  └─────────────────┘            │
│                                 │
├─────────────────────────────────┤
│  MessageInput (输入框)           │
│  [🎤] [文字输入框...] [😊] [📷]  │
└─────────────────────────────────┘
```

### 4.4.4 组件设计

#### ConversationItem组件（会话项）
```typescript
interface ConversationItemProps {
  conversation: Conversation;
  onClick: (id: string) => void;
}

export const ConversationItem: React.FC<ConversationItemProps> = ({
  conversation,
  onClick
}) => {
  return (
    <div
      className={styles.conversationItem}
      onClick={() => onClick(conversation.id)}
    >
      <Avatar src={conversation.avatar} size={48} />

      <div className={styles.content}>
        <div className={styles.header}>
          <span className={styles.name}>{conversation.name}</span>
          <span className={styles.time}>
            {SSTimeUtil.relative(conversation.lastMessageAt)}
          </span>
        </div>

        <div className={styles.footer}>
          <span className={styles.lastMessage}>
            {conversation.lastMessage}
          </span>
          {conversation.unreadCount > 0 && (
            <Badge count={conversation.unreadCount} />
          )}
        </div>
      </div>
    </div>
  );
};
```

#### MessageBubble组件（消息气泡）
```typescript
interface MessageBubbleProps {
  message: Message;
  isMine: boolean;
}

export const MessageBubble: React.FC<MessageBubbleProps> = ({
  message,
  isMine
}) => {
  const renderContent = () => {
    switch (message.type) {
      case 'text':
        return <div className={styles.text}>{message.content}</div>;
      case 'image':
        return <img src={message.content} alt="" />;
      case 'video':
        return <video src={message.content} controls />;
      case 'audio':
        return <AudioPlayer src={message.content} />;
      default:
        return null;
    }
  };

  return (
    <div className={classNames(styles.messageBubble, {
      [styles.mine]: isMine,
      [styles.other]: !isMine
    })}>
      {!isMine && <Avatar src={message.sender.avatar} size={36} />}

      <div className={styles.bubble}>
        {renderContent()}
        <div className={styles.status}>
          {message.status === 'read' && isMine && '已读'}
        </div>
      </div>

      {isMine && <Avatar src={message.sender.avatar} size={36} />}
    </div>
  );
};
```

#### MessageInput组件（消息输入框）
```typescript
export const MessageInput: React.FC = () => {
  const [text, setText] = useState('');
  const [showToolbar, setShowToolbar] = useState(false);

  const handleSend = async () => {
    if (!text.trim()) return;

    await sendMessage({ type: 'text', content: text });
    setText('');
  };

  return (
    <div className={styles.messageInput}>
      <Button icon={<MicIcon />} onClick={() => {/* 语音 */}} />

      <Input
        value={text}
        onChange={setText}
        placeholder="请输入消息..."
        onFocus={() => setShowToolbar(false)}
      />

      <Button icon={<EmojiIcon />} onClick={() => setShowToolbar(!showToolbar)} />
      <Button icon={<ImageIcon />} onClick={() => {/* 图片 */}} />

      {text ? (
        <Button type="primary" onClick={handleSend}>发送</Button>
      ) : null}

      {showToolbar && (
        <Toolbar
          onImage={() => {/* 选择图片 */}}
          onVideo={() => {/* 选择视频 */}}
          onFile={() => {/* 选择文件 */}}
        />
      )}
    </div>
  );
};
```

### 4.4.5 状态管理（messageSlice.ts）

```typescript
interface MessageState {
  conversations: Conversation[];
  currentConversation: string | null;
  messages: {
    [conversationId: string]: Message[];
  };
  unreadCount: number;
  onlineUsers: string[];
  typing: {
    [conversationId: string]: string[];
  };
}
```

### 4.4.6 WebSocket事件

```typescript
// services/socket/events.ts
export enum SocketEvent {
  // 连接事件
  CONNECT = 'connect',
  DISCONNECT = 'disconnect',

  // 消息事件
  MESSAGE_NEW = 'message:new',
  MESSAGE_READ = 'message:read',
  MESSAGE_DELIVERED = 'message:delivered',
  MESSAGE_RECALLED = 'message:recalled',

  // 用户事件
  USER_ONLINE = 'user:online',
  USER_OFFLINE = 'user:offline',
  USER_TYPING = 'user:typing'
}

// 消息发送
socket.emit('message:send', {
  conversationId,
  type: 'text',
  content: 'Hello'
});

// 接收新消息
socket.on('message:new', (message: Message) => {
  dispatch(addMessage(message));
});
```

---

## 4.5 我的模块

### 4.5.1 模块概述

个人中心模块展示用户信息、个人动态、好友列表，提供账号设置、隐私管理等功能。

### 4.5.2 功能清单

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 个人信息展示 | P0 | 头像、昵称、签名等 |
| 我的动态 | P0 | 发布的所有动态 |
| 我的好友 | P0 | 好友列表 |
| 我的关注 | P1 | 关注的用户/话题 |
| 我的粉丝 | P1 | 粉丝列表 |
| 编辑资料 | P0 | 修改个人信息 |
| 账号设置 | P0 | 密码、绑定手机等 |
| 隐私设置 | P1 | 谁可以看、黑名单 |
| 通用设置 | P1 | 通知、语言等 |
| 关于我们 | P2 | 版本、协议等 |

### 4.5.3 页面结构

```
┌─────────────────────────────────┐
│  ProfileHeader (个人信息卡片)     │
│  ┌───────────────────────────┐  │
│  │  [背景图]                  │  │
│  │  ┌─────┐                  │  │
│  │  │头像  │ 张三              │  │
│  │  └─────┘ 这是我的个性签名  │  │
│  │  [关注 100] [粉丝 200]     │  │
│  │  [编辑资料]                │  │
│  └───────────────────────────┘  │
├─────────────────────────────────┤
│  Stats (数据统计)                │
│  ┌────┐ ┌────┐ ┌────┐          │
│  │ 50 │ │ 20 │ │ 30 │          │
│  │动态│ │收藏│ │赞过│          │
│  └────┘ └────┘ └────┘          │
├─────────────────────────────────┤
│  MenuList (功能菜单)             │
│  [👥 我的好友]        [>]        │
│  [⭐ 我的收藏]        [>]        │
│  [🏷️ 我的话题]        [>]        │
│  ───────────────────────────    │
│  [⚙️ 设置]            [>]        │
│  [❓ 帮助与反馈]      [>]        │
│  [ℹ️ 关于]            [>]        │
│  ───────────────────────────    │
│  [退出登录]                      │
└─────────────────────────────────┘
```

### 4.5.4 组件设计

#### UserInfo组件（用户信息卡片）
```typescript
interface UserInfoProps {
  user: User;
  onEdit: () => void;
}

export const UserInfo: React.FC<UserInfoProps> = ({ user, onEdit }) => {
  return (
    <div className={styles.userInfo}>
      <div
        className={styles.background}
        style={{ backgroundImage: `url(${user.backgroundImage})` }}
      />

      <div className={styles.content}>
        <Avatar src={user.avatar} size={80} />

        <div className={styles.info}>
          <h2>{user.nickname}</h2>
          <p>{user.signature || '这个人很懒，什么都没留下'}</p>

          <div className={styles.stats}>
            <div className={styles.stat}>
              <span className={styles.count}>{user.followingCount}</span>
              <span className={styles.label}>关注</span>
            </div>
            <div className={styles.stat}>
              <span className={styles.count}>{user.followerCount}</span>
              <span className={styles.label}>粉丝</span>
            </div>
          </div>

          <Button onClick={onEdit}>编辑资料</Button>
        </div>
      </div>
    </div>
  );
};
```

#### Settings组件（设置页面）
```typescript
export const Settings: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const handleLogout = async () => {
    const confirmed = await SSDialog.confirm({
      title: '确认退出',
      content: '确定要退出登录吗？'
    });

    if (confirmed) {
      await authApi.logout();
      dispatch(logout());
      navigate('/login');
    }
  };

  return (
    <div className={styles.settings}>
      <NavBar back title="设置" />

      <List>
        <List.Item
          prefix={<UserIcon />}
          onClick={() => navigate('/settings/account')}
          arrow
        >
          账号设置
        </List.Item>

        <List.Item
          prefix={<LockIcon />}
          onClick={() => navigate('/settings/privacy')}
          arrow
        >
          隐私设置
        </List.Item>

        <List.Item
          prefix={<BellIcon />}
          onClick={() => navigate('/settings/notification')}
          arrow
        >
          通知设置
        </List.Item>

        <List.Item
          prefix={<InfoIcon />}
          onClick={() => navigate('/settings/about')}
          arrow
        >
          关于我们
        </List.Item>
      </List>

      <div className={styles.logoutSection}>
        <Button block danger onClick={handleLogout}>
          退出登录
        </Button>
      </div>
    </div>
  );
};
```

### 4.5.5 状态管理（profileSlice.ts）

```typescript
interface ProfileState {
  userInfo: User | null;
  myPosts: Post[];
  myFriends: User[];
  favorites: Post[];
  followedTopics: Topic[];
  stats: {
    postCount: number;
    favoriteCount: number;
    likeCount: number;
  };
}
```

### 4.5.6 API接口

```typescript
// services/api/user.ts
export const userApi = {
  // 获取个人信息
  getProfile: () =>
    axios.get<User>('/api/v1/users/me'),

  // 更新个人信息
  updateProfile: (data: UpdateProfileDto) =>
    axios.put<User>('/api/v1/users/me', data),

  // 上传头像
  uploadAvatar: (file: File) =>
    axios.post<string>('/api/v1/users/avatar', file),

  // 获取我的动态
  getMyPosts: (params: { page: number }) =>
    axios.get<Post[]>('/api/v1/users/me/posts', { params }),

  // 获取我的好友
  getMyFriends: () =>
    axios.get<User[]>('/api/v1/users/me/friends')
};
```

---

## 5. 路由设计

### 5.1 路由配置

```typescript
// router/routes.tsx
import { lazy } from 'react';

// 懒加载页面组件
const Home = lazy(() => import('@/views/Home'));
const Community = lazy(() => import('@/views/Community'));
const Publish = lazy(() => import('@/views/Publish'));
const Message = lazy(() => import('@/views/Message'));
const Profile = lazy(() => import('@/views/Profile'));

export const routes = [
  {
    path: '/',
    element: <MainLayout />,
    children: [
      {
        index: true,
        element: <Navigate to="/home" replace />
      },
      {
        path: 'home',
        element: <Home />
      },
      {
        path: 'community',
        element: <Community />,
        children: [
          {
            path: 'topic/:id',
            element: <TopicDetail />
          }
        ]
      },
      {
        path: 'publish',
        element: <Publish />,
        children: [
          { path: 'text', element: <TextPost /> },
          { path: 'image', element: <ImagePost /> },
          { path: 'video', element: <VideoPost /> }
        ]
      },
      {
        path: 'message',
        element: <Message />,
        children: [
          {
            path: 'chat/:id',
            element: <ChatRoom />
          },
          {
            path: 'group/:id',
            element: <GroupChat />
          }
        ]
      },
      {
        path: 'profile',
        element: <Profile />,
        children: [
          { path: 'edit', element: <EditProfile /> },
          { path: 'posts', element: <MyPosts /> },
          { path: 'friends', element: <MyFriends /> },
          { path: 'settings', element: <Settings /> }
        ]
      }
    ]
  },
  {
    path: '/auth',
    element: <AuthLayout />,
    children: [
      { path: 'login', element: <Login /> },
      { path: 'register', element: <Register /> }
    ]
  },
  {
    path: '/post/:id',
    element: <PostDetail />
  },
  {
    path: '/user/:id',
    element: <UserProfile />
  },
  {
    path: '*',
    element: <NotFound />
  }
];
```

### 5.2 路由守卫

```typescript
// router/AuthGuard.tsx
export const AuthGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { isAuthenticated } = useSelector((state: RootState) => state.auth);
  const location = useLocation();

  if (!isAuthenticated) {
    return <Navigate to="/auth/login" state={{ from: location }} replace />;
  }

  return <>{children}</>;
};
```

---

## 6. 状态管理

### 6.1 Store配置

```typescript
// store/index.ts
import { configureStore } from '@reduxjs/toolkit';
import authReducer from './slices/authSlice';
import homeReducer from './slices/homeSlice';
import communityReducer from './slices/communitySlice';
import publishReducer from './slices/publishSlice';
import messageReducer from './slices/messageSlice';
import profileReducer from './slices/profileSlice';

export const store = configureStore({
  reducer: {
    auth: authReducer,
    home: homeReducer,
    community: communityReducer,
    publish: publishReducer,
    message: messageReducer,
    profile: profileReducer
  },
  middleware: (getDefaultMiddleware) =>
    getDefaultMiddleware({
      serializableCheck: false
    })
});

export type RootState = ReturnType<typeof store.getState>;
export type AppDispatch = typeof store.dispatch;
```

### 6.2 Redux Hooks

```typescript
// store/hooks.ts
import { TypedUseSelectorHook, useDispatch, useSelector } from 'react-redux';
import type { RootState, AppDispatch } from './index';

export const useAppDispatch = () => useDispatch<AppDispatch>();
export const useAppSelector: TypedUseSelectorHook<RootState> = useSelector;
```

---

## 7. 组件库设计

### 7.1 布局组件

#### TabBar（底部导航栏）
```typescript
export const TabBar: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const tabs = [
    { key: 'home', title: '首页', icon: <HomeIcon /> },
    { key: 'community', title: '社区', icon: <CommunityIcon /> },
    { key: 'publish', title: '发布', icon: <AddIcon /> },
    { key: 'message', title: '消息', icon: <MessageIcon />, badge: 5 },
    { key: 'profile', title: '我的', icon: <UserIcon /> }
  ];

  return (
    <div className={styles.tabBar}>
      {tabs.map(tab => (
        <div
          key={tab.key}
          className={classNames(styles.tab, {
            [styles.active]: location.pathname.includes(tab.key)
          })}
          onClick={() => navigate(`/${tab.key}`)}
        >
          <Badge content={tab.badge}>
            {tab.icon}
          </Badge>
          <span>{tab.title}</span>
        </div>
      ))}
    </div>
  );
};
```

### 7.2 功能组件

#### SSDialog（统一弹窗）
```typescript
// components/SSDialog/index.tsx
export const SSDialog = {
  confirm: (options: ConfirmOptions): Promise<boolean> => {
    return Dialog.confirm({
      ...options,
      confirmText: options.confirmText || '确定',
      cancelText: options.cancelText || '取消'
    });
  },

  alert: (options: AlertOptions): Promise<void> => {
    return Dialog.alert({
      ...options,
      confirmText: options.confirmText || '知道了'
    });
  },

  toast: (message: string, type: 'success' | 'error' | 'warning' = 'success') => {
    Toast.show({
      icon: type === 'success' ? 'success' : type === 'error' ? 'fail' : 'loading',
      content: message
    });
  },

  loading: (show: boolean, text = '加载中...') => {
    if (show) {
      Toast.show({
        icon: 'loading',
        content: text,
        duration: 0
      });
    } else {
      Toast.clear();
    }
  }
};
```

---

## 8. 工具类设计

所有工具类按照开发规则使用 `SS` 前缀命名，详见 `.claude/development-rules.md` 文档 4.4节。

主要工具类：
- `SSTimeUtil` - 时间处理
- `SSStringUtil` - 字符串处理
- `SSValidateUtil` - 数据验证
- `SSStorageUtil` - 本地存储
- `SSDictUtil` - 字典管理
- `SSFileUtil` - 文件处理
- `SSImageUtil` - 图片处理

---

## 9. API接口设计

### 9.1 接口规范

所有接口遵循RESTful设计规范，详见技术架构文档。

### 9.2 接口分类

```typescript
// services/api/index.ts
export * from './auth';      // 认证相关
export * from './user';      // 用户相关
export * from './post';      // 动态相关
export * from './topic';     // 话题相关
export * from './message';   // 消息相关
export * from './upload';    // 上传相关
```

---

## 10. 开发规范

严格遵循 `.claude/development-rules.md` 文档中的所有规范：

- ✅ 自定义类文件使用SS前缀
- ✅ UI/请求/逻辑三层分离
- ✅ 函数不超过50行，组件不超过200行
- ✅ 子组件在子目录，通用组件在上级
- ✅ 充分封装工具类和公共组件
- ✅ TypeScript严格模式
- ✅ 代码提交前检查清单

---

## 11. 开发计划

### 11.1 第一阶段（Week 1-2）：基础架构

- [ ] 项目初始化、目录结构搭建
- [ ] 工具类封装（6大工具类）
- [ ] 通用组件开发（Layout、Dialog、Loading等）
- [ ] 路由配置、状态管理配置
- [ ] API服务层封装

### 11.2 第二阶段（Week 3-4）：认证与首页

- [ ] 登录注册模块
- [ ] 首页内容流
- [ ] 内容互动（点赞、评论）
- [ ] 下拉刷新、上拉加载

### 11.3 第三阶段（Week 5-6）：社区与发布

- [ ] 社区话题列表
- [ ] 话题详情页
- [ ] 发布模块（文字、图片、视频）
- [ ] 图片上传、视频上传

### 11.4 第四阶段（Week 7-9）：消息模块

- [ ] 会话列表
- [ ] 聊天室界面
- [ ] WebSocket连接
- [ ] 实时消息收发
- [ ] 消息类型支持（文字、图片、语音）

### 11.5 第五阶段（Week 10-11）：个人中心

- [ ] 个人信息展示
- [ ] 我的动态
- [ ] 我的好友
- [ ] 设置页面
- [ ] 编辑资料

### 11.6 第六阶段（Week 12）：优化与测试

- [ ] 性能优化
- [ ] 兼容性测试
- [ ] Bug修复
- [ ] 上线准备

---

**文档版本**: v1.0
**创建日期**: 2025-11-06
**最后更新**: 2025-11-06
**维护人员**: 前端开发团队
**审核状态**: 待审核
